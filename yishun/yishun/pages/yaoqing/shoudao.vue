		...
		<scroll-view scroll-y="true" style="height: 1250upx;">
			<view v-for="(item,index) in information" :key="index">
				<view class="yuepai">
					...
					<view class="location">
						<view class="mincheng">
							邀请人:{{information[index].invitedPeople}}
						</view>
						<view class="didian">
							<view>
					...
					<view class="anniu">
						<button @click="jieshou(index)" class="anniuyangshi yaoqing" type="default">接受邀请</button>
						<button @click="jujue(index)" class="anniuyangshi guanzhu" type="default">拒绝邀请</button>
					</view>
				</view>
			</view>
		</scroll-view>
		...
		methods: {
			...
			jieshou(e){
				const res = this.$myRequest({
					url: '/appointment/acceptInvitation',
					data: {
						inviteAccount:this.information[e].inviteAccount,
						appoId:this.information[e].appointmentId
					}
				});
				setTimeout(function(){
					uni.redirectTo({
					    url: '../yaoqing/shoudao?inviteAccount=1',
					});
				},100)
			},
			jujue(e){
				const res = this.$myRequest({
					url: '/appointment/refuseInvitation',
					data: {
						inviteAccount:this.information[e].inviteAccount,
						appoId:this.information[e].appointmentId
					}
